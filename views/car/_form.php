...
<div class="car-form">
				...
				<div class="item panel panel-default"><!-- widgetBody -->
					...
					<div class="panel-body">
						...
						<div class="row">
							...
							<div class="col-sm-6">
								<img src="<?= Yii::getAlias('/web/files/images/').$CarInside->car_id.'/'.$CarInside->image?>" class="image-preview img-rounded" alt="..." style="width: 170px; height: auto;"/>
							</div>
						</div><!-- end:row -->
					</div>
				</div>
	...
	<?php DynamicFormWidget::begin([
		'widgetContainer' => 'dynamicform_wrapper_drivers', // required: only alphanumeric characters plus "_" [A-Za-z0-9_]
		'widgetBody' => '.container-items-drivers', // required: css class selector
		'widgetItem' => '.item', // required: css class
		'limit' => 6, // the maximum times, an element can be cloned (default 999)
		'min' => 1, // 0 or 1 (default 1)
		'insertButton' => '.add-item', // css class
		'deleteButton' => '.remove-item', // css class
		'model' => $model->drivers[0],
		'formId' => 'dynamic-form',
		'formFields' => [
//			'id',
			'driver_id'
	...
	<div class="panel panel-default">
		<div class="panel-heading">
			<i class="fa fa-picture-o"></i> Водители
			<button type="button" class="pull-right add-item btn btn-success btn-xs"><i class="fa fa-plus"></i> Добавить водителя</button>
			<div class="clearfix"></div>
		</div>
		<div class="panel-body container-items-drivers"><!-- widgetContainer -->
			<?php foreach ($model->drivers as $index => $driver): ?>
				<div class="item panel panel-default"><!-- widgetBody -->
					<div class="panel-heading">
						<span class="panel-title-address">Водитель</span>
						<button type="button" class="pull-right remove-item btn btn-danger btn-xs"><i class="fa fa-minus"></i></button>
						<div class="clearfix"></div>
					</div>
					<div class="panel-body">
						...
						if (!$driver->isNewRecord) {
							echo Html::activeHiddenInput($driver, "[{$index}]id");
						}
						?>

						<div class="row">
							<div class="col-sm-6">
								<?= $form->field($driver, "[{$index}]driver_id")->dropDownList(\app\models\Driver::getDriversForDropdownList(), [
									'prompt' => '-- Выберите --'
								]) ?>
							</div>
						</div><!-- end:row -->
