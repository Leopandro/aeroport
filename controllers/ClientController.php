				...
				'rules' => [
					[
						...
						'matchCallback' => function(){
							return (Yii::$app->user->identity->role_id == User::ROLE_ADMIN || Yii::$app->user->identity->role_id == User::ROLE_ACCOUNTANT);
						}
					],
				],
			],
...
//	{
//		if (!(Yii::$app->user->identity->role_id == '1'|| Yii::$app->user->identity->role_id == '4') )
...
//	}
	...
	public function actionIndex()
	{
		...
		$searchModel  = Yii::createObject(ClientSearch::className());
		$dataProvider = $searchModel->search(Yii::$app->request->get());

		if (Yii::$app->user->identity->role_id == \app\models\User::ROLE_ACCOUNTANT)
		{
			return $this->render('accountant_index', [
...
//		        $user->setAttribute('role_id',2);
			if (!$user->getAttribute('admin_id'))
				$user->setAttribute('admin_id', Yii::$app->user->identity->getId());
			$userInfo->load(Yii::$app->request->post());
			$userInfo->id = $user->id;
			if ($userInfo->validate() && $user->save())
			{
//				$e = $userInfo->getErrors();
//				$userInfo->isNewRecord = false;
//				$r = $userInfo->save();
...
//        if ($profile->days)
//            $profile->days= json_decode($profile->days);
...
//        $this->trigger(self::EVENT_BEFORE_PROFILE_UPDATE, $event);
		...
		if ($profile->load(Yii::$app->request->post())) {
			$profile->days = json_encode($profile->days);
			if ($profile->save())
